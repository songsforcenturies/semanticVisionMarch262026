...
from models import (
    ...
    TargetRegion,
    Affiliate, AffiliateReferral, AffiliateSettings, AffiliateRewardType,
    BrandOffer, UserOfferPreference, generate_affiliate_code,
)
from auth import (
...
# Create the main app
app = FastAPI(title="Semantic Vision API", version="1.0.0")
...
async def get_patent_filing_2026_pdf():
    ...
    from fastapi.responses import FileResponse
    pdf_path = Path(__file__).parent.parent / "PATENT_FILING_FINAL_v4.pdf"
    if not pdf_path.exists():
...
async def get_patent_filing_2026_md():
    ...
    from fastapi.responses import FileResponse
    md_path = Path(__file__).parent.parent / "PATENT_FILING_FINAL_v4.md"
...
@api_router.post("/affiliates/signup")
async def affiliate_signup(data: dict = Body(...)):
    ...
    try:
        ...
        if resend_key:
            ...
            affiliate_link = f"{os.environ.get('FRONTEND_URL', 'https://parent-partner.preview.emergentagent.com')}/register?ref={affiliate.affiliate_code}"
            resend.Emails.send({
...
@api_router.get("/affiliates/my-stats")
...
@api_router.put("/brands/offers/{offer_id}")
...
@api_router.delete("/brands/offers/{offer_id}")
...
@api_router.put("/offers/preferences")
...
@api_router.post("/offers/{offer_id}/click")
